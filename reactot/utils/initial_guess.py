"""
OT-FM 초기 구조(x_0) 생성 모듈 — Combo [AE] (IDPP + Stochastic ensemble).

본 모듈은 idea2 조합 [AE] (= IDPP + ensemble)를 위해 다음을 통합한다:
...
def generate_stochastic_x0_ensemble(
    ...
    """Per-atom Gaussian perturbation ensemble. Returns (K, N, 3)."""
...
def select_best_from_ensemble(
    ...
    """Pick best TS from ensemble. method ∈ {midpoint_distance, rmsd_consensus}."""
...
def _fallback_hop_weights(pos_R, pos_P, atomic_numbers,
    ...
    """Hop-distance fallback for Idea 1 weights."""
...
def compute_x0_AE(
    ...
    Args:
        pos_R, pos_P, atomic_numbers: standard
        K: ensemble size (k=0 is the deterministic IDPP base)
        sigma_base: peripheral atom perturbation scale (Å)
        seed: reproducibility
        idpp_kwargs: forwarded to compute_idpp; default uses 200 iterations
            with halogen-aware clash penalty
        atom_weights: Idea 1-A weights; None → hop-distance fallback
        core_weight_clip: weights above this -> perturbation 0
