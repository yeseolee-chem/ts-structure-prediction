...
class EnSB(nn.Module):
    ...
    def __init__(
        ...
        # Combo [AE]: x_0 generator config.
        # x0_method ∈ {"midpoint", "idpp", "linear", "idpp_clash", "a_e"}.
        valid_x0 = {"midpoint", "idpp", "linear", "idpp_clash", "a_e"}
    ...
    def sample_batch(
        ...
        if self.mapping == "R->P":
            ...
            if self.mapping_initial == 'RP':
                # Combo [AE]: route through x0_method ('midpoint' preserves
    ...
    def sample(self, x1, representations, conditions,
               ...
               x1_override=None):
        """Reverse-time sampler. ``x1_override`` (Tensor, optional) replaces
        the x1 computed by sample_batch with the supplied tensor (after
        ts_mask CoG removal) — entry point for the [AE] stochastic ensemble.
        """
        ...
        # Combo [AE]: stochastic-ensemble entry. CoG-removed using the
        # same ts_mask sample_batch already used so the per-fragment
        # CoG invariant holds.
    ...
    def ode_sampling_ensemble(
        ...
        """
        Combo [AE]: drive the reverse-time sampler K times, once per
        perturbed x1 in ``x1_ensemble`` (each tensor produced by
        compute_x0_AE row-k). Returns a list of K predicted-TS tensors at
        t=0, detached on CPU.
        """
        ts_ensemble = []
