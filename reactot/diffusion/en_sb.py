    ...
    def forward(
        ...
        Args:
            atom_weights_prior: optional (n_target_atoms,) per-atom prior weights
                from Idea 1-A (graph-distance exp-decay). Used as the KL
                regularization target when ``dynamics.learn_importance`` is on.
                When the dynamics does not learn importance, these weights are
                applied directly to the FM loss (fixed-weight Idea 1-A mode),
                giving full backward-compatibility with a cb-A checkpoint.
        ...
        if importance_per_frag is not None:
            # --- Idea 1-D: learned per-atom importance + KL-to-prior ---
            ...
            # --- Fixed-weight fallback (Idea 1-A behavior) ---
