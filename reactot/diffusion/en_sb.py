...
class EnSB(nn.Module):
    ...
    def __init__(
        ...
        # Combo [BDE]: x_0 generator config.
    ...
    def sample_batch(
        ...
        if self.mapping == "R->P":
            ...
            if self.mapping_initial == 'RP':
                # Combo [BDE]: route through x0_method (midpoint preserves
                # the legacy (R+P)/2 behaviour).
    ...
    def sample(self, x1, representations, conditions,
        ...
        x1 — entry point for Combo [BDE] stochastic ensemble."""
    ...
    def ode_sampling_ensemble(
        ...
        """
        Combo [BE]/[BDE]: drive sampler K times with perturbed x1 candidates.
        """
        ts_ensemble = []
