...
class EnSB(nn.Module):
    ...
    def __init__(
        ...
        # Combo [FE]: x_0 generator config.
        valid_x0 = {"midpoint", "idpp", "linear", "learned", "f_e"}
        if x0_method not in valid_x0:
            raise ValueError(
                f"x0_method={x0_method!r} not in {sorted(valid_x0)}"
            )
        if x0_method in ("learned", "f_e") and not learned_x0_checkpoint:
            raise ValueError(
                f"x0_method={x0_method!r} requires learned_x0_checkpoint"
            )
    ...
    def sample_batch(
        ...
        if self.mapping == "R->P":
            ...
            if self.mapping_initial == 'RP':
                # Combo [FE]: route through x0_method.
    ...
    def sample(self, x1, representations, conditions,
        ...
        x1 — entry point for Combo [FE] stochastic ensemble."""
    ...
    def ode_sampling_ensemble(
        ...
        """Combo [FE]: drive sampler K times with perturbed x1 candidates."""
        ts_ensemble = []
