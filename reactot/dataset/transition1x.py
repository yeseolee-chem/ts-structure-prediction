...
class ProcessedTS1x(BaseDataset):
    def __init__(
        ...
        **kwargs,
        ...
        # Combo [AD]: optional cache directory for xTB-refined x_0 (npy files
        # keyed by reaction index). Populated by scripts/precompute_x0_xtb.py.
    ...
    def _load_x0(self, idx, pos_R, pos_P, atomic_numbers):
        """
        Combo [AD]: Load a pre-computed x_0 (xTB-refined) from cache, falling
        back to the (R+P)/2 midpoint if no cache entry exists. See
        scripts/precompute_x0_xtb.py for cache generation.
        """
