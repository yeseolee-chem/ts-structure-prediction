...
class EGNNDynamics(BaseDynamics):
    def __init__(
        ...
        # and outputs one scalar per atom. Downstream (en_sb.py) squashes this
        # with sigmoid to get a [w_min, 1.0] weight for the FM loss, and
        # KL-regularizes it against the graph-distance prior.
    ...
    def forward(
        ...
        Returns:
            (xh_final, edge_attr_final, importance_per_frag):
                xh_final is the per-fragment list of concatenated [vel, h].
                edge_attr_final is the optional edge tensor.
                importance_per_frag is None unless ``learn_importance=True``,
                otherwise a list matching ``fragment_names`` with one
                ``(n_atoms_frag,)`` logits tensor per fragment.
        """
        pos = torch.concat(
        ...
        # Idea 1-D: extract per-atom importance logits from the LEFTNet node
        # features *before* the condition slice, since those features still
        # carry the full in_hidden_channels dimension expected by the head.
        if self.learn_importance:
            importance_logits = self.importance_head(h_final).squeeze(-1)  # (N_total,)
        else:
