"""
scripts/precompute_x0_xtb.py — Combo [BD] x_0 cache generator.

Runs Stage 1 (IC interpolation) + Stage 2 (GFN2-xTB short refinement)
on each (R, P) and saves the resulting x_0 to disk. The cache directory
is consumed by ProcessedTS1x via ``x0_cache_dir``.
...
사용법:
    python scripts/precompute_x0_xtb.py \\
        ...
        --output_path data/x0_xtb_cache_BD/ \\
        --n_workers 4 \\
        --interpolate b_d
"""
import argparse
