"""
scripts/precompute_x0_xtb.py — Combo [BD]/[BDE] x_0 cache generator.

For each reaction, runs Stage 1 (IC interpolation) + Stage 2 (GFN2-xTB
short refinement) and saves the resulting x_0 to disk for the cache
consumed by ProcessedTS1x via ``x0_cache_dir``.

xtb-python is process-safe (Pool OK) but not thread-safe.

사용법:
    python scripts/precompute_x0_xtb.py \\
        --data_path PATH_TO_DATASET \\
        --output_path data/x0_xtb_cache_BDE/ \\
        --n_workers 4 \\
        --interpolate b_d
"""
import argparse
...
def process_single_reaction(args):
    ...
    try:
        if interpolate == "b_d":
            ...
            x0 = compute_ic_interpolation(pos_R, pos_P, atomic_numbers)
        return idx, x0, True
...
def main():
    ...
    parser.add_argument('--interpolate', default='b_d',
                        choices=['b_d', 'ic'])
