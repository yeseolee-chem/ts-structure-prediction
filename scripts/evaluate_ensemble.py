"""
scripts/evaluate_ensemble.py — Combo [FE] K vs sigma ablation.
"""
import argparse
import json
import time
...
def evaluate_one_setting(model, dataset, K, sigma, learned_x0_checkpoint, device):
    from reactot.run_model import predict_ts_FE
    ...
    for sample in dataset:
        ...
        t0 = time.time()
        result = predict_ts_FE(
            model, learned_x0_checkpoint, pos_R, pos_P, z,
            K=K, sigma_base=sigma, device=device,
        )
...
def main():
    ...
    parser.add_argument('--learned_x0_checkpoint', required=True)
    ...
    for K in args.K_values:
        for sigma in args.sigma_values:
            ...
            results.append(evaluate_one_setting(
                model, dataset, K, sigma,
                args.learned_x0_checkpoint, args.device,
            ))
