"""
scripts/evaluate_ensemble.py — Combo [E]/[AE]/[ADE] K vs sigma ablation.

Sweep K and sigma_base; for each setting compute mean / median / std of
TS RMSD and the wall-clock time. Loads model+dataset via project-specific
hooks (TODO replace placeholders to wire up to your evaluation
pipeline).
...
def evaluate_one_setting(model, dataset, K, sigma):
    from reactot.run_model import predict_ts_ADE
    ...
    for sample in dataset:
        ...
        charge = sample.get('charge', 0)

        t0 = time.time()
        result = predict_ts_ADE(
            model, pos_R, pos_P, z,
            K=K, sigma_base=sigma, charge=charge,
        )
...
def main():
    ...
    # === TODO: load model + validation dataset for the project ===
    model = None
    dataset = None
